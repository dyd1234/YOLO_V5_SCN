...
# pair up the 
import val_rotate_One4All_extra as validate_rotate  # for end-of-epoch mAP
...
def train(hyp, opt, device, callbacks):
    ...
    dot.render("model_graph_one4all")        # "model_graph" 是文件名，将生成 model_graph.pdf
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # angle = 0 # to test what is going to have 
            # angle = random.uniform(0, 360) # anti clock-wise
            # angle = np.random.uniform(0, 360)
            # if angle < 0.5 and angle > -0.5:
            #     print(f"The value of the angle is {angle} at the batch {i}")
            # if angle < 90.5 and angle > 89.5:
            #     print(f"The value of the angle is {angle} at the batch {i}")

            # if angle < 180.5 and angle > 179.5:
            #     print(f"The value of the angle is {angle} at the batch {i}")
            # if angle < 270.5 and angle > 269.5:
            #     print(f"The value of the angle is {angle} at the batch {i}")
            # angle = random.uniform(-180, 180) # anti clock-wise
            # angle = random.uniform(-30, 30) # anti clock-wise

            # fill=(114, 114, 114) costs so many times
            # imgs = TF.rotate(imgs, -angle, fill=(114, 114, 114)) # to clock wise 
            imgs = TF.rotate(imgs, -angle) # to clock wise # 看看是不是这两个有问题

            # seems a bit different from the other stuff
            _, targets = fixed_perspective_batch_v2( # right wi
            # imgs, _ = fixed_perspective_batch_v2a( # right wi 
            
            # _, targets = fixed_perspective_batch( # right with version 1
                imgs,
                targets,
                angle # dont konw why .....
            ) # 
            ...
            with torch.cuda.amp.autocast(amp):
                pred = model(imgs)  # forward  original YOLO v5s
    ...
    np.save('angles.npy', np.array(angles_list))  # 保存为 .npy 文件
    if RANK in {-1, 0}:
        LOGGER.info(f"\n{epoch - start_epoch + 1} epochs completed in {(time.time() - t0) / 3600:.3f} hours.")
        # get the 
        print(f"if the val_loader is {val_loader}")
        for f in last, best:
            if f.exists():
                strip_optimizer(f)  # strip optimizers
                if f is best:
                    LOGGER.info(f"\nValidating {f}...") 
                    # get the input of the YOLO to make the
                    # results, _, _ = validate_rotate.run( # run_final_test
                    results, _, _ = validate_rotate.run_final_test( # run_final_test
                        data_dict,
                        batch_size=batch_size // WORLD_SIZE * 2,
                        imgsz=imgsz,
...
def parse_opt(known=False):
    ...
    parser.add_argument("--cos-lr", action="store_true", help="cosine LR scheduler", default=True)
