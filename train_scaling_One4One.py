...
# The name of the file is val_scaling_One4One.py, but the function is validate_scale
import val_scaling_One4One as validate_scale  # for end-of-epoch mAP
# import val_scaling as validate_scale  # for end-of-epoch mAP
import val as validate  # for end-of-epoch mAP
from models.experimental import attempt_load
from models.yolocopy_1 import Model
...
# very important to make the One4One work
def train(hyp, opt, device, callbacks, fixed_scaling=1.0): # set default to 0 
    """
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # do img transformations for the other parts
            # angle = 0
            # angle = random.uniform(0, 360) # anti clock-wise
            scale = fixed_scaling # 
            if scale != 1.0: # if angle is not 0, then we do not need to rotate the image
            ...
            if RANK in {-1, 0}:
                ...
                # callbacks.run("on_train_batch_end_scn", model, ni, imgs, targets, paths, list(mloss), Hyper_X)
        ...
        if RANK in {-1, 0}:
            ...
            if not noval or final_epoch:  # Calculate mAP
                results, maps, _ = validate_scale.run(
                    data_dict,
    ...
    if RANK in {-1, 0}:
        ...
        for f in last, best:
            if f.exists():
                ...
                if f is best:
                    LOGGER.info(f"\nValidating {f}...")
                    results, _, _, nt, tp, fp, p, r, f1, ap, ap50, ap_class = validate_scale.run_final_test( # The result of the final test
                        data_dict,
                        batch_size=batch_size // WORLD_SIZE * 2,
                        imgsz=imgsz,
    ...
    return results, nt, tp, fp, p, r, f1, ap, ap50, ap_class
...
def main(opt, callbacks=Callbacks()):
    ...
    # Train
    if not opt.evolve: # try here with differnet angles
        # The metrics results should be attached here
        # Initialize metrics arrays as what I defined here
        mp_nparray, mr_nparray, map50_nparray, map_nparray = [], [], [], []
        tp_nparray, fp_nparray, p_nparray, r_nparray, f1_nparray, ap50_nparray = [], [], [], [], [], []

        # then follow every thing to train it 
        # angle_list = [0, 15, 30, 45, 60, 75, 90] # should be ok? 
        fixed_scales = [0.2, 0.5, 1.0, 1.5, 1.8] # is this ok? 
        # angle_list = [0, 15] # should be ok? 

        for fixed_scaling in fixed_scales: # 
            result, nt, tp, fp, p, r, f1, ap, ap50, ap_class = train(opt.hyp, opt, device, callbacks, fixed_scaling=fixed_scaling) # see what is going to have int results
            # attach the thing 
            # train(opt.hyp, opt, device, callbacks, fixed_angle=15)

            # print(result)
            mp, mr, map50, map, *other_metrics = result
