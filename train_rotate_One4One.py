...
import val_rotate_One4One as validate_rotate  # for end-of-epoch mAP
...
def train(hyp, opt, device, callbacks, fixed_angle=0): # set default to 0 
    """
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            angle = fixed_angle # 
            if angle != 0: # if angle is not 0, then we do not need to rotate the image
                imgs = TF.rotate(imgs, -angle) # to clock wise 

                _, targets = fixed_perspective_batch_v2( # right
                    imgs,
                    targets,
                    angle # dont konw why .....
                ) # 
    ...
    if RANK in {-1, 0}:
        ...
        for f in last, best:
            if f.exists():
                ...
                if f is best:
                    LOGGER.info(f"\nValidating {f}...")
                    results, _, _, nt, tp, fp, p, r, f1, ap, ap50, ap_class = validate_rotate.run_final_test( # The result of the final test
                        data_dict,
                        batch_size=batch_size // WORLD_SIZE * 2,
                        imgsz=imgsz,
...
def parse_opt(known=False):
    ...
    parser.add_argument("--fixed_angle", type=float, default=0.0, help="One4One fixed angle from the outer input")
...
def main(opt, callbacks=Callbacks()):
    ...
    if not opt.evolve: # try here with differnet angles
        ...
        # for fixed_angle in angle_list:
        fixed_angle = opt.fixed_angle
        print(f"The input angle fron the outer is {fixed_angle}")
        result, nt, tp, fp, p, r, f1, ap, ap50, ap_class = train(opt.hyp, opt, device, callbacks, fixed_angle=fixed_angle) # see what is going to have int results
        # attach the thing 
