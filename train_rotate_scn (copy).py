...
import val_rotate_scn as validate_rotate  # for end-of-epoch mAP
# import val_scaling as validate_scale  # for end-of-epoch mAP
import val as validate  # for end-of-epoch mAP
from models.experimental import attempt_load
from models.yolo import Model, Model_SCN
...
from utils.general import ( # pre-defined data_argumentation
    reset_bn_stats_scn_rotate,
)
...
def train(hyp, opt, device, callbacks): # 
    ...
    if pretrained:
        ...
        model = Model_SCN(cfg or ckpt["model"].yaml, ch=3, nc=nc, anchors=hyp.get("anchors"), hin=2).to(device)  # create
        for param in model.parameters(): # 
            assert param.device == device, f"Parameter {param} not on {device}"
        for buffer in model.buffers():
            assert buffer.device == device, f"Buffer {buffer} not on {device}"
        ...
        model = Model_SCN(cfg, ch=3, nc=nc, anchors=hyp.get("anchors"), hin=2).to(device)  # create rotation
        for param in model.parameters():
            assert param.device == device, f"Parameter {param} not on {device}"
        for buffer in model.buffers():
            assert buffer.device == device, f"Buffer {buffer} not on {device}"

    print("The model is showing below")
    print(model)
    ...
    cos = nn.CosineSimilarity(dim=0, eps=1e-6) # do not use this in the rotation case

    # EMA
    ema = ModelEMA(model) if RANK in {-1, 0} else None # check if we need to do something more?
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # angle = random.uniform(0, 90) # anti clock-wise try this one with less rotations


            imgs = TF.rotate(imgs, -angle) # to clock wise 

            # _, targets = fixed_perspective_batch( # right
            _, targets = fixed_perspective_batch_v2(
                imgs,
                targets,
                angle # dont konw why .....
            ) # 
