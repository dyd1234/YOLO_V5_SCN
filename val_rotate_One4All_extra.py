...
from utils.general import (
    ...
    xywh2xyxy,
    xyxy2xywh,
    transform_angle, # new functions
    # debug_hook, # check the inner contents of YOLO model itself, such as checking the 
)
from utils.metrics import ConfusionMatrix, ap_per_class, box_iou, ap_per_class_rotate
from utils.plots import output_to_target, plot_images, plot_val_study
from utils.torch_utils import select_device, smart_inference_mode

from utils.augmentations import ( # pre-defined data_argumentation
    Albumentations,
    augment_hsv,
    ...
    mixup,
    random_perspective,
    fixed_perspective_batch, # 
    fixed_perspective_batch_v2,
)
...
def run( # to cooprate with training
    data,
    ...
    print("I am in the val_rotate.py")
    """
    ...
    if training:  # called by train.py
        print(" I am in the part of the in-epoch validation") 
        device, pt, jit, engine = next(model.parameters()).device, True, False, False  # get model device, PyTorch model
        half &= device.type != "cpu"  # half precision only supported on CUDA
        print(f"The value of the half is {half}")
        model.half() if half else model.float()

    else:  # called directly
        device = select_device(device, batch_size=batch_size)
        print(" I am in the part of the final tests") 
        # Directories
        ...
        print(f"The value of the half is {half} in the final test")
        if engine:
    ...
    # Dataloader
    if not training: # 如果不是从训练的时候进行调用的
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        # angle = random.choice([0, 90, 180, 270])  # 从直接角度中随机选择 # check here later for the moment
        # angle = random.uniform(-180, 180) # anti clock-wis
        # angle = 0 # anti clock-wise

        im = TF.rotate(im, -angle) # to clock wise

        _, targets = fixed_perspective_batch_v2( # right with rotation is each batch 
        # _, targets = fixed_perspective_batch( # right with rotation is each batch 
                im,
                targets, 
                angle # dont konw why .....
            ) # 

        # Hyper_X = transform_angle(angle).to(device) # check if we have to change to fp16

        # if half == True:
        #     Hyper_X = Hyper_X.half()

        # do the transformations before this part --------------------------------
        with dt[0]: # 
            if cuda:
        ...
        with dt[1]:
            ...
            # preds, train_out = model(im, Hyper_X) if compute_loss else (model(im, augment=augment), None)
...
def run_final_test( # 
    ...
    for angle in range(0, 360, 5):  # 角度从0到359，每15度一个间隔
    # for angle in range(-180, 180, 5):  # 角度从0到359，每5度一个间隔
    # for angle in range(0, 360, 90):  # 角度从0到359，每15度一个间隔
    # for angle in range(-30, 30, 5):  # 角度从0到359，每5度一个间隔
        print(f"The value of the angle is {angle}")
        print(f"The value of half is {half}")
        jdict, stats, ap, ap_class = [], [], [], []
        print(f"The value of the jdict is {jdict} and the type of the jdict is {type(jdict)}")
        print(f"The value of the stats is {stats} and the type of the stats is {type(stats)}")
        print(f"The value of the ap is {ap} and the type of the ap is {type(ap)}")
        print(f"The value of the ap_class is {ap_class} and the type of the ap_class is {type(ap_class)}")
        seen = 0
        ...
        for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
            ...
            # This is better than the version v1
            _, targets = fixed_perspective_batch_v2(  # Apply fixed perspective transformation
            # _, targets = fixed_perspective_batch( # right with rotation is each batch 
                im,
                targets,
                angle
            ...
            # Plot images
            if plots and batch_i < 3:
                plot_images(im, targets, paths, save_dir / f"val_batch{batch_i}_labels_angle{angle}.jpg", names)  # labels
                plot_images(im, output_to_target(preds), paths, save_dir / f"val_batch{batch_i}_pred_angle{angle}.jpg", names)  # pred

            callbacks.run("on_val_batch_end", batch_i, im, targets, paths, shapes, preds)

        # Compute metrics
        # stats = [torch.cat([torch.tensor(x) if isinstance(x, np.ndarray) else x for x in sublist], 0).cpu().numpy() for sublist in zip(*stats)]  # to numpy
        stats = [torch.cat(x, 0).cpu().numpy() for x in zip(*stats)]  # to numpy
        if len(stats) and stats[0].any():
            tp, fp, p, r, f1, ap, ap_class = ap_per_class_rotate(*stats, plot=plots, save_dir=save_dir, names=names, angle=angle) # check the ap_per_class_rotate function
            ap50, ap = ap[:, 0], ap.mean(1)  # AP@0.5, AP@0.5:0.95
            mp, mr, map50, map = p.mean(), r.mean(), ap50.mean(), ap.mean()
        nt = np.bincount(stats[3].astype(int), minlength=nc)  # number of targets per class
        ...
        # Plots
        if plots:
            # confusion_matrix.plot(save_dir=save_dir / f"confusion_matrix_angle{angle}", names=list(names.values()))
            confusion_matrix.plot_check_others(save_dir=save_dir / f"confusion_matrix_angle{angle}", names=list(names.values()))
            callbacks.run("on_val_end", nt, tp, fp, p, r, f1, ap, ap50, ap_class, confusion_matrix)
        ...
        if save_json and len(jdict): # 
            ...
            if not os.path.exists(anno_json):
                anno_json = os.path.join(data["path"], "annotations", "instances_val2017.json")
            pred_json = str(save_dir / f"{w}_predictions_angle{angle}.json")  # predictions
            LOGGER.info(f"\nEvaluating pycocotools mAP... saving {pred_json}...")
            with open(pred_json, "w") as f:
                json.dump(jdict, f)
        ...
        # Append metrics to numpy arrays
        mp_nparray.append(mp)
        mr_nparray.append(mr)
        map50_nparray.append(map50)
        map_nparray.append(map)
        tp_nparray.append(tp)
        fp_nparray.append(fp)
        p_nparray.append(p)
        r_nparray.append(r)
        f1_nparray.append(f1)
        ap50_nparray.append(ap50)

    # Save metrics to npy files in the save_dir directory
    np.save(save_dir / 'mp.npy', mp_nparray)
    np.save(save_dir / 'mr.npy', mr_nparray)
    np.save(save_dir / 'map50.npy', map50_nparray)
    np.save(save_dir / 'map.npy', map_nparray)
    np.save(save_dir / 'tp.npy', tp_nparray)
    np.save(save_dir / 'fp.npy', fp_nparray)
    np.save(save_dir / 'p.npy', p_nparray)
    np.save(save_dir / 'r.npy', r_nparray)
    np.save(save_dir / 'f1.npy', f1_nparray)
    np.save(save_dir / 'ap50.npy', ap50_nparray)

    # Calculate average metrics
    avg_mp = np.mean(mp_nparray)
    avg_mr = np.mean(mr_nparray)
    avg_map50 = np.mean(map50_nparray)
    avg_map = np.mean(map_nparray)
    avg_tp = np.mean(tp_nparray)
    avg_fp = np.mean(fp_nparray)
    avg_p = np.mean(p_nparray)
    avg_r = np.mean(r_nparray)
    avg_f1 = np.mean(f1_nparray)
    avg_ap50 = np.mean(ap50_nparray)

    return (avg_mp, avg_mr, avg_map50, avg_map, avg_tp, avg_fp, avg_p, avg_r, avg_f1, avg_ap50), _, t
