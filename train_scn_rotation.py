...
import val_rotation as validate_rotate  # for end-of-epoch mAP
import val_scaling as validate_scale  # for end-of-epoch mAP
...
def save_images_with_boxes(imgs, targets, paths, save_dir="saved_images"):
    os.makedirs(save_dir, exist_ok=True)  # 创建保存图像的目录

    for i, img in enumerate(imgs):
        fig, ax = plt.subplots(1)
        img = img.permute(1, 2, 0).cpu().numpy()  # 将图像从CHW转换为HWC格式
        h, w = img.shape[:2]  # 获取图像的宽度和高度
        ax.imshow(img)

        for target in targets:
            if int(target[0]) == i:  # 确保bbox与当前图像匹配
                cls_id = int(target[1])  # 类别ID
                bbox = target[2:].cpu().numpy()  # bbox坐标 (xywh)
                ...
                # 添加类别标签
                ax.text(x1, y1, f'Class {cls_id}', color='white', fontsize=12, 
                        bbox=dict(facecolor='red', alpha=0.5))

        ax.axis('off')  # 关闭坐标轴
        image_save_path = os.path.join(save_dir, os.path.basename(paths[i]))
        plt.savefig(image_save_path)  # 保存图像到文件
...
def train(hyp, opt, device, callbacks): # 
    ...
    # if pretrained: # we do not use this part
    #     print("Yes I am using the pretrained weight")
    #     with torch_distributed_zero_first(LOCAL_RANK):
    #         weights = attempt_download(weights)  # download if not found locally
    #     ckpt = torch.load(weights, map_location="cpu")  # load checkpoint to CPU to avoid CUDA memory leak
    #     model = Model(cfg or ckpt["model"].yaml, ch=3, nc=nc, anchors=hyp.get("anchors"), hin=2).to(device)  # create
       
    #    # for loading the full weights of the list
       
    #     exclude = ["anchor"] if (cfg or hyp.get("anchors")) and not resume else []  # exclude keys
    #     csd = ckpt["model"].float().state_dict()  # checkpoint state_dict as FP32

    #     csd = intersect_dicts(csd, model.state_dict(), exclude=exclude)  # intersect #
    #     # Loading the backbone weights into the model
    #     # csd = intersect_dicts(backbone_weights, model.state_dict(), exclude=[exclude])  # intersect

    #     load_info = model.load_state_dict(csd, strict=False)  # load 
    ...
    #     for k in load_info.loaded_tensors.keys():
    #         print(k)

    #     LOGGER.info(f"Transferred {len(csd)}/{len(model.state_dict())} items from {weights}")  # report
    # else:
    #     model = Model(cfg, ch=3, nc=nc, anchors=hyp.get("anchors"), hin=2).to(device)  # create rotation
    
    # works fine
    check_suffix(weights, ".pt")  # check weights
    pretrained = weights.endswith(".pt")
    if pretrained:  # if using pretrained weights
        print("Yes, I am using the pretrained weight")
        with torch_distributed_zero_first(LOCAL_RANK):
            weights = attempt_download(weights)  # download if not found locally
        ckpt = torch.load(weights, map_location="cpu")  # load checkpoint to CPU to avoid CUDA memory leak
        ...
        #     layer.register_forward_hook(lambda module, input, output: debug_hook(module, input, output, model_name=model_name))
        ...
        for k, v in csd.items():
            if k in model.state_dict() and torch.equal(model.state_dict()[k], v.to(device)):  # Move `v` to the model's device
                print(k)
    ...
    # amp = check_amp(model)  # check AMP_
    amp = False  # check AMP_

    print(f"The number of the amp is {amp}") # 

    # Freeze freeze with dedicated layers
    freeze = [f"model.{x}." for x in (freeze if len(freeze) > 1 else range(freeze[0]))]  # layers to freeze

    # 鎵撳嵃瑕佸喕缁撶殑灞傚悕
    print(f"------------------Layers to freeze: {freeze}")

    for k, v in model.named_parameters(): #銆�杩欓噷闇�瑕佽瀺鍚堜竴涓�
        # 鎵撳嵃褰撳墠灞傜殑鍚嶇О鍜屾槸鍚﹂渶瑕佽缁冪殑鏍囧織
        # print(f"Processing layer: {k}, requires_grad before: {v.requires_grad}")
        
        v.requires_grad = True  # train all layers

        # 鎵撳嵃鏇存柊 requires_grad 鍚庣殑鐘舵��
        # print(f"Layer: {k}, requires_grad after setting to True: {v.requires_grad}")
        
        if any(x in k for x in freeze):
            LOGGER.info(f"freezing {k}")
            v.requires_grad = False  # freeze this layer
            
            # 鍐荤粨鍚庢墦鍗�
            # print(f"Layer: {k} has been frozen, requires_grad after: {v.requires_grad}")
    ...
    train_loader, dataset = create_dataloader( # at lease do not try to make scaling and rotarion works
        train_path,
        imgsz,
...
#　start to make all training loop # 
    # Start training
    t0 = time.time()
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # Can be set here to make the 

            angle = random.uniform(0, 360) # anti clock-wise

            # save_images_with_boxes(imgs, targets, paths, save_dir="saved_images_before")

            imgs = TF.rotate(imgs, -angle) # to clock wise

            # if i == 0:
            #     break

            # print(f"--------------------The type of the imgs and targets are {}")
            # print(f"\n--------------The type of the im and targets are {type(imgs)} and {type(targets)}")
            # print(f"\n--------------The type of the im and targets are {imgs.shape} and {type(targets.dim())}")
            # print(f"\n-------------- {targets.numpy()}")
            # print(f"\n-------------- {type(imgs.numpy())}")
            # print(f"\n-------------- {batch_size}")
            # print(f"\n-------------- {imgs.numpy()}")
            # print(f"-----------The numer of i is {i}")

            _, targets = fixed_perspective_batch( # right
                imgs,
                targets,
                angle # dont konw why .....
            ) # 

            # save_images_with_boxes(imgs, targets, paths)

            # return _ # try with some tests
            ...
            # angle = random.uniform(0, 360) # should that be 0~360?
            # X, y = X.to(device), y.to(device)

            # print("-------------------------------------------------------")
            # print(f"The type of image is {type(imgs)}")
            # print(imgs.shape)
            # print(targets)


            # imgs_out, targets_out = fixed_perspective( # should be verified for more details
            #     imgs,
            #     targets, 
            #     degrees=angle, # 
            # )

            # plot_images_with_boxes(imgs, targets, paths)
            # save_images_with_boxes(imgs_out, targets_out, paths) # try here to make the result great


            # show contents with only one bbox
            # if i == 0:
            #     break
            ...
            # for target in targets:
            #     x1, y1, x2, y2 = map(int, target[1:5])
            #     cv2.rectangle(imgs, (x1, y1), (x2, y2), (0, 255, 0), 2)  # 绿色框
                
            # # show images with bbox
            # plt.imshow(imgs[:, :, ::-1])  # BGR to RGB
            ...
            # save_image_with_boxes(imgs, targets, title='After Perspective Transformation')
            # save_image_with_boxes(imgs, targets, 'after_perspective.png', title='After Perspective Transformation')
            ...
            with torch.cuda.amp.autocast(amp): #　
                # print(f"The number of AMP is {amp}")
                # print(f"\n--------------The type of the im and targets are {imgs.dtype} and {type(targets)}")
                pred = model(imgs, Hyper_X)  # forward the amp for this model is not fully OK
                # do not use the quartic loss function
        ...
        if RANK in {-1, 0}: # 
            ...
            if not noval or final_epoch:  # Calculate mAP
                results, maps, _ = validate_rotate.run( # run_rotation with the validation 
                    data_dict,
...
def parse_opt(known=False):
    ...
    parser.add_argument("--scn_dimension", type=int, default=1, help="Set the SCN layer dimension of the whole YOLO model")
