...
def run_final_test(
    ...
    # for angle in range(0, 360, 15):  # 角度从0到359，每15度一个间隔 
    for scale in np.arange(0.2, 1.8, 0.1):  # 
        print(f"The value of the angle is {scale}")
        ...
        if plots:
            ...
            confusion_matrix.plot_check_others(save_dir=save_dir / f"confusion_matrix_angle{scale}", names=list(names.values()))
            callbacks.run("on_val_end", nt, tp, fp, p, r, f1, ap, ap50, ap_class, confusion_matrix)
