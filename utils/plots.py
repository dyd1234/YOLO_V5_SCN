...
def feature_visualization(x, module_type, stage, n=32, save_dir=Path("runs/detect/exp")):
    ...
    if ("Detect" not in module_type) and (
        "Segment" not in module_type) and (
        "Detect_SCN" not in module_type
...
# scn method of visualizaion, not very important
def feature_visualization_scn(x, hyper_x, module_type, stage, n=32, save_dir=Path("runs/detect/exp")):
    """
    x:              Features to be visualized
    module_type:    Module type
    stage:          Module stage within model
