...
# how to make the ap_per_class function to calculate the ap for each class and each angle?
...
def ap_per_class_rotate(tp, conf, pred_cls, target_cls, plot=False, save_dir=".", names=(), eps=1e-16, prefix="", angle=None):
    """
    Compute the average precision, given the recall and precision curves.
    ...
    # Arguments
        ...
        plot:  Plot precision-recall curve at mAP@0.5
        save_dir:  Plot save directory
        angle:  Angle or scale factor to include in the saved plot filenames
    # Returns
        The average precision as computed in py-faster-rcnn.
    """
    # Sort by objectness
    ...
    names = dict(enumerate(names))  # to dict
    if plot:
        angle_suffix = f"_angle{angle}" if angle is not None else ""
        plot_pr_curve(px, py, ap, Path(save_dir) / f"{prefix}PR_curve{angle_suffix}.png", names)
        plot_mc_curve(px, f1, Path(save_dir) / f"{prefix}F1_curve{angle_suffix}.png", names, ylabel="F1")
        plot_mc_curve(px, p, Path(save_dir) / f"{prefix}P_curve{angle_suffix}.png", names, ylabel="Precision")
        plot_mc_curve(px, r, Path(save_dir) / f"{prefix}R_curve{angle_suffix}.png", names, ylabel="Recall")
...
class ConfusionMatrix:
    ...
    # --------------------------------------------------------------------------------------------------
    @TryExcept("WARNING ⚠️ ConfusionMatrix plot_check_others failure")
    def plot_check_others(self, normalize=True, save_dir="", names=()):
        """Plots confusion matrix using seaborn, optional normalization; can save plot to specified directory."""
        import seaborn as sn
        from pathlib import Path
