...
class Loggers:
    ...
    def on_train_batch_end_scn(self, model, ni, imgs, targets, paths, vals, hyper_x=None):
        """Logs training batch end events, plots images, and updates external loggers with batch-end data."""
        log_dict = dict(zip(self.keys[:3], vals))
        # Callback runs on train batch end
        # ni: number integrated batches (since train start)
        if self.plots:
            if ni < 3:
                ...
                plot_images(imgs, targets, paths, f)
                if ni == 0 and self.tb and not self.opt.sync_bn:
                    log_tensorboard_graph_scn(self.tb, model, imgsz=(self.opt.imgsz, self.opt.imgsz), hyper_x=hyper_x)
            if ni == 10 and (self.wandb or self.clearml):
                files = sorted(self.save_dir.glob("train*.jpg"))
                if self.wandb:
                    self.wandb.log({"Mosaics": [wandb.Image(str(f), caption=f.name) for f in files if f.exists()]})
...
# def log_tensorboard_graph_scn(tb, model, imgsz=(640, 640), hyper_x=None):
#     """Logs the model graph to TensorBoard with specified image size and model."""
#     try:
#         p = next(model.parameters())  # for device, type
#         imgsz = (imgsz, imgsz) if isinstance(imgsz, int) else imgsz  # expand
...
#         with warnings.catch_warnings():
#             warnings.simplefilter("ignore")  # suppress jit trace warning
#             tb.add_graph(torch.jit.trace(de_parallel(model, hyper_x), im, strict=False), [])
#     except Exception as e:
#         LOGGER.warning(f"WARNING ⚠️ TensorBoard graph visualization failure {e}")

def log_tensorboard_graph_scn(tb, model, imgsz=(640, 640), hyper_x=None): # 
    """Logs the model graph to TensorBoard with specified image size and model, including hyper_x."""
    try:
        p = next(model.parameters())  # for device, type
        imgsz = (imgsz, imgsz) if isinstance(imgsz, int) else imgsz  # expand
        im = torch.zeros((1, 3, *imgsz)).to(p.device).type_as(p)  # input image (WARNING: must be zeros, not empty)
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")  # suppress jit trace warning
            # Use a lambda function to pass both im and hyper_x to the model
            tb.add_graph(torch.jit.trace(lambda x: model(x, hyper_x), im, strict=False), [])
    except Exception as e:
        LOGGER.warning(f"WARNING ⚠️ TensorBoard graph visualization failure in scn {e}")
