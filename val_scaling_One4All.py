...
def run( # to cooprate with training
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        # scale = 0 # anti clock-wise

        # im = TF.rotate(im, -scale) # to clock wise
        im = TF.affine(im, scale=scale, angle=0, translate=(0, 0), shear=0.0) # 

        _, targets = fixed_perspective_batch_v3( # right
            im,
            targets,
            scale=scale # dont konw why .....
        ) # 
...
def run_final_test(
    ...
    for scale in np.arange(0.2, 1.8, 0.1):  # 
    # for angle in range(0, 360, 90):  # 
        print(f"The value of the scale is {scale}")
        ...
        for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
            ...
            # im = TF.rotate(im, -scale)
            im = TF.affine(im, scale=scale, angle=0, translate=(0, 0), shear=0.0) # 

            # This is better than the version v1， but only work for the scaling
            _, targets = fixed_perspective_batch_v3(  # Apply fixed perspective transformation
                im,
                targets,
                scale=scale # dont konw why .....
            )
            ...
            # Plot images
            if plots and batch_i < 3:
                plot_images(im, targets, paths, save_dir / f"val_batch{batch_i}_labels_angle{scale}.jpg", names)  # labels
                plot_images(im, output_to_target(preds), paths, save_dir / f"val_batch{batch_i}_pred_angle{scale}.jpg", names)  # pred
        ...
        stats = [torch.cat(x, 0).cpu().numpy() for x in zip(*stats)]  # to numpy
        if len(stats) and stats[0].any():
            tp, fp, p, r, f1, ap, ap_class = ap_per_class_rotate(*stats, plot=plots, save_dir=save_dir, names=names, angle=scale) # check the ap_per_class_rotate function
            ap50, ap = ap[:, 0], ap.mean(1)  # AP@0.5, AP@0.5:0.95
            mp, mr, map50, map = p.mean(), r.mean(), ap50.mean(), ap.mean()
        nt = np.bincount(stats[3].astype(int), minlength=nc)  # number of targets per class
        ...
        if plots:
            ...
            confusion_matrix.plot_check_others(save_dir=save_dir / f"confusion_matrix_scale{scale}", names=list(names.values()))
            callbacks.run("on_val_end", nt, tp, fp, p, r, f1, ap, ap50, ap_class, confusion_matrix)
        ...
        if save_json and len(jdict): # 
            ...
            if not os.path.exists(anno_json):
                anno_json = os.path.join(data["path"], "annotations", "instances_val2017.json")
            pred_json = str(save_dir / f"{w}_predictions_angle{scale}.json")  # predictions
            LOGGER.info(f"\nEvaluating pycocotools mAP... saving {pred_json}...")
            with open(pred_json, "w") as f:
                json.dump(jdict, f)
