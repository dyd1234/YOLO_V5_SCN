...
def run(
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        # angle = fixed_angle # anti clock-wise
        scale = fixed_scaling
...
fixed_scaling=1.0,
    ...
    print("I am in the final test of val_rotate.py")
    """
    Evaluates a YOLOv5 model on a dataset and logs performance metrics.
    ...
    if len(stats) and stats[0].any():
        # tp, fp, p, r, f1, ap, ap_class = ap_per_class(*stats, plot=plots, save_dir=save_dir, names=names)
        tp, fp, p, r, f1, ap, ap_class = ap_per_class_rotate(*stats, plot=plots, save_dir=save_dir, names=names, angle=scale)
        ap50, ap = ap[:, 0], ap.mean(1)  # AP@0.5, AP@0.5:0.95
        mp, mr, map50, map = p.mean(), r.mean(), ap50.mean(), ap.mean()
    nt = np.bincount(stats[3].astype(int), minlength=nc)  # number of targets per class
    ...
    # Plots
    if plots: # check if this is correct
        # confusion_matrix.plot(save_dir=save_dir / f"confusion_matrix_angle{angle}", names=list(names.values()))
        confusion_matrix.plot_check_others(save_dir=save_dir / f"confusion_matrix_angle{fixed_scaling}", names=list(names.values()))
        callbacks.run("on_val_end", nt, tp, fp, p, r, f1, ap, ap50, ap_class, confusion_matrix)
    ...
    for i, c in enumerate(ap_class):
        maps[c] = ap[i]
    return (mp, mr, map50, map, *(loss.cpu() / len(dataloader)).tolist()), maps, t, nt, tp, fp, p, r, f1, ap, ap50, ap_class
