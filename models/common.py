...
# here I added some SCN based stuff to make the stuff running ok
...
# ---------------------------- end of the defination of the new SCN type
...
# SCN part for the number

class Conv_SCN2(nn.Module):
    def __init__(self, dimensions, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True, is_scn=True):
        ...
        if self.is_scn:
            self.conv_weight_list = self.create_param_combination_conv2d(dimensions, c1, c2, self.kernel_size)
        else:
            self.conv = nn.Conv2d(c1, c2, k, s, self.padding, groups=g, bias=False)
        
        self.bn = nn.BatchNorm2d(c2, eps=0.001, momentum=0.03)
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())

    # fp32 bit with other steps
    # 
    ...
    def create_param_combination_conv2d(self, dimensions: int, in_channels: int, out_channels: int, kernel_size: int = 3) -> nn.ParameterList:
        ...
        for _ in range(dimensions): 
            weight = Parameter(torch.empty((out_channels, in_channels, kernel_size, kernel_size), dtype=torch.float32))
            init.kaiming_uniform_(weight, a=math.sqrt(5))
            weight_list.append(weight) 
        return weight_list

    def calculate_weighted_sum(self, param_list: List[Parameter], coefficients: torch.Tensor) -> torch.Tensor:
        # Ensure coefficients are in float32
        coefficients = coefficients.to(dtype=torch.float32)
        with autocast(enabled=False):  # Disable autocast to ensure fp32 calculations
            weighted_list = [a * b for a, b in zip(param_list, coefficients)]
            return torch.sum(torch.stack(weighted_list), dim=0)

    def execute_hyper_conv2d(self, x: torch.Tensor, weight_list: List[Parameter], coefficients: torch.Tensor, stride: int = 1, padding: int = 0, dilation: int = 1, groups: int = 1) -> torch.Tensor:
        # print(f"----------x device: {x.device}, weight_list device: {weight_list[0].device}, bias_list device: {None}, coefficients device: {coefficients.device}")  # 调试信息
        weights = self.calculate_weighted_sum(weight_list, coefficients)
        with autocast(enabled=False):  # Disable autocast to ensure fp32 calculations
            return F.conv2d(x, weight=weights, stride=stride, padding=padding, dilation=dilation, groups=groups)
    
    def forward(self, x, hyper_x):
        with autocast(enabled=False):  # Disable autocast to ensure fp32 calculations
            hyper_x = hyper_x.to(dtype=torch.float32)  # Ensure hyper_x is in fp32
            if self.is_scn:
                # print("I am in the process of SCN_CONV") 
                x = self.execute_hyper_conv2d(x, self.conv_weight_list, hyper_x, stride=self.stride, padding=self.padding, groups=self.groups)
            else:
                x = self.conv(x)
            x = self.bn(x)
            x = self.act(x)
    ...
    def forward_fuse(self, x, hyper_x):
        with autocast(enabled=False):  # Disable autocast to ensure fp32 calculations
            if self.is_scn:
                x = self.execute_hyper_conv2d(x, self.conv_weight_list, hyper_x, stride=self.stride, padding=self.padding, groups=self.groups)
            else:
...
# no need anymore
class Conv_SCN_B(nn.Module): # why this part is not working?
    def __init__(self, dimensions, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True, is_scn=True):
        super().__init__()
        self.is_scn = is_scn
        self.kernel_size = k
        ...
        self.groups = g
        self.requires_hyper_x = True
        self.c2 = c2 # try if this is OK

        if self.is_scn:
            self.conv_weight_list = self.create_param_combination_conv2d(dimensions, c1, c2, self.kernel_size)
            self.bias_list = self.create_param_combination_bias(dimensions, c2)
        else:
            self.conv = nn.Conv2d(c1, c2, k, s, self.padding, groups=g, bias=True)  # Always using bias
        
        self.bn = nn.BatchNorm2d(c2, eps=0.001, momentum=0.03)
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
        
        # # Initialize biases
        # self._initialize_biases() # initial the weights
           # Initialize biases if SCN is used
        if self.is_scn:
    ...
    def forward(self, x, hyper_x):
        with autocast():
            if self.is_scn:
                x = self.execute_hyper_conv2d(x, self.conv_weight_list, self.bias_list, hyper_x, stride=self.stride, padding=self.padding, groups=self.groups)
            else:
                x = self.conv(x)
            x = self.bn(x)
            x = self.act(x)
...
# Add SCN Bottleneck
class Bottleneck_SCN(nn.Module): #　The　stuff is ok
    def __init__(self, dimensions, c1, c2, shortcut=True, g=1, e=0.5, is_scn=False):
        super().__init__()
        self.is_scn = is_scn
        c_ = int(c2 * e)
        self.cv1 = Conv_SCN(dimensions, c1, c_, 1, 1, is_scn=is_scn)
        self.cv2 = Conv_SCN(dimensions, c_, c2, 3, 1, g=g, is_scn=is_scn)
        self.add = shortcut and c1 == c2

    def forward(self, x, hyper_x):
        out = x + self.cv2(self.cv1(x, hyper_x), hyper_x) if self.add else self.cv2(self.cv1(x, hyper_x), hyper_x)
        return out
    
# C3 SCN stuff
class C3_SCN(nn.Module):
    def __init__(self, dimensions, c1, c2, n=1, shortcut=True, g=1, e=0.5, is_scn=True): # 
        super().__init__()
        self.is_scn = is_scn
        self.requires_hyper_x = True # needed for the outer loop 

        c_ = int(c2 * e) # 
        self.cv1 = Conv_SCN(dimensions, c1, c_, 1, 1, is_scn=is_scn) #
        self.cv2 = Conv_SCN(dimensions, c1, c_, 1, 1, is_scn=is_scn) #
        self.cv3 = Conv_SCN(dimensions, 2 * c_, c2, 1, is_scn=is_scn) #
        self.m = nn.ModuleList([Bottleneck_SCN(dimensions, c_, c_, shortcut, g, e=1.0, is_scn=is_scn) for _ in range(n)])

    def forward(self, x, hyper_x): 
        return self.cv3(torch.cat(
            (
                self._forward_module_list(self.m, self.cv1(x, hyper_x), hyper_x),
                self.cv2(x, hyper_x)
            ), dim=1), hyper_x)

    def _forward_module_list(self, module_list, x, hyper_x):
...
class SPPF_SCN(nn.Module): # 
    def __init__(self, dimensions, c1, c2, k=5, is_scn=True): # 
        ...
        self.cv2 = Conv_SCN(dimensions, c_ * 4, c2, 1, 1, is_scn=self.is_scn)
        self.m = nn.MaxPool2d(kernel_size=k, stride=1, padding=k // 2)

    def forward(self, x, hyper_x):
        x = self.cv1(x, hyper_x)
        with warnings.catch_warnings():
