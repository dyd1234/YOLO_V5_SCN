...
# 
import val_scaling_scn as validate_rotate  # for end-of-epoch mAP
...
def train(hyp, opt, device, callbacks):
    ...
    if pretrained:
        ...
        model = Model_SCN(cfg or ckpt["model"].yaml, ch=3, nc=nc, anchors=hyp.get("anchors"), hin=1).to(device)  # create
        ...
        model = Model_SCN(cfg, ch=3, nc=nc, anchors=hyp.get("anchors"), hin=1).to(device)  # create rotation
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # Hyper_X = transform_angle(scale).to(device) # 
            ...
            with torch.cuda.amp.autocast(amp):
                ...
                # beta1 = model.hyper_stack(Tensor([scale]).to(device))
                # scale2 = random.uniform(0.2, 1.8)
                # beta2 = model.hyper_stack(Tensor([scale2]).to(device))

                # loss += 0.1*pow(cos(beta1, beta2), 2) # 
                # try with use cos loss
